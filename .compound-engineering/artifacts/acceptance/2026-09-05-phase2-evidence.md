...
### Merge preparation: CI dependency correction

The first GitHub run for [PR #44](https://github.com/ThomsenDrake/Cestus/pull/44), [run 33981778216](https://github.com/ThomsenDrake/Cestus/actions/runs/33981778216), passed typecheck but failed nine PDF-dependent assertions because the clean Ubuntu runner lacked `pdfinfo`/`pdftotext`. The workflow now installs the documented `poppler-utils` prerequisite before verification. Product behavior and assertions are unchanged; no PDF tests are skipped or mocked. All 17 tests across the four affected files passed locally with Poppler. Final remote check results are attached to the PR; this dependency correction does not change the pending real-record/live-provider acceptance.
