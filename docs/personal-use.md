...
The browser extracts proposed entities, typed facts, relationships and occurrences from selected passages using the persisted `investigation.v1` vocabulary and exact quotations. Its result is **unreviewed**, not an accepted investigative fact. Historical Phase 2 summary operations and artifacts remain readable. It uses an OpenAI-compatible chat-completions HTTP endpoint; provider compatibility and live accounting must be evaluated against the endpoint you choose. No provider is selected when configuration is missing.
...
5. Review the value next to its quotations, then **Accept** or **Reject**. **Sources and history …** opens the evidence, review decisions and memberships. **Open source passage** opens the immutable extraction at the saved location. A model score is only the provider's score. Unknown vocabulary cannot be accepted; edit it into a supported proposal instead.
