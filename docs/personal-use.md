...
2. Under **External document processing**, select up to 24 passages (at most 32 KiB of selected passage data), enter a **Maximum budget (USD)** and output-token limit, then **Preview exact transfer**. Review the outgoing text/instructions, source and extraction hashes, destination/model, the **Reviewed extraction vocabulary** snapshot, operation, byte volume, conservative token estimate, operator-supplied prices, and limits.
