...
19 manifest identity/canonical hash, and every selectable entry's identity,
path, type, size, digest or protected commitment, and review state. It reports
...
When the selection contains an image or PDF, the preview and approval also bind
the exact selected image/PDF entry identities, hashes, types, and bytes; Mistral
...
automatic Mistral OCR of the exact selected image/PDF bytes. It does not
...
selected image/PDF; and absence of cancellation or a terminal entry state.
...
file while digest and byte count are checked. Protected source bytes are never
...
After `prepared`, Cestus atomically installs the verified staging file into
canonical content-addressed blob storage. An existing blob is reused only after
...
bytes, policy, destination, or provenance. A prepared record without matching
staged bytes is a hard integrity failure and cannot reopen the source. Blob
mismatch, ledger conflict, destination replacement, or impossible provenance
stops safely.
...
- Crash before `prepared` leaves no evidence. Crash after `prepared` resumes
  ...
  prepared staging and mismatched existing blobs fail integrity checks.
