...
string value. Public-only JWK/JWKS remains ordinary byte-identical JSON.
...
The activated Specification 17 worker receives semantic text candidates with
checked source-coordinate mappings. It may affect content only for these six
...
Candidates are tokenized once and processed in deterministic overlapping
windows bounded by the verified model context limit. Versioned window size,
overlap, tokenizer, and ownership intervals give each source position one
canonical result. A detected span that cannot be reconstructed completely
across window boundaries causes quarantine.
...
- `packages/local-runtime/src/**` only for authenticated manually sanitized
  replacement preview/decision and dependency wiring through existing
  ingestion services.
