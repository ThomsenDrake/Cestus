# Specification 16A-R2a — Superseded Pure Secret Commitment Protocol

Status: superseded by approved replacement Specifications 16A-R2a-R2.1 through
16A-R2a-R2.3.

Specification 16A-R2a is not an implementation input. Its complete public protocol,
product, and safety requirements are preserved by the Specification 16 umbrella and
the R2.1 through R2.3 replacement specifications. The failed implementation candidate is
history only and must not be inspected, reused, merged, amended, or pushed.

The replacements execute serially before Specification 16A-R2b. Specifications
16A-R2b through 16D remain blocked until R2.1 through R2.3 are integrated and their
required verification is accepted.
