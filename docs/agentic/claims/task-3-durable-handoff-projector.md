...
- Fourth review-fix RED: added coverage for duplicate `agent.specialist-handoff.recorded` events on the same `preparedEventId`. The targeted command failed because exact duplicate recorded retries produced two handoffs instead of one; the same test also covers changed `verifiedAt` as a conflicting recorded readback.
- Fourth review-fix GREEN: `npm test -- packages/agent/test/specialist-handoff-projection.test.ts packages/agent/test/projection.test.ts packages/agent/test/cockpit.test.ts` passed with 3 test files and 40 tests after deduping exact recorded retries and failing closed on changed recorded readback payloads.
- Fourth review-fix verification: `npm run verify` passed: typecheck; 172 passed test files, 3 skipped; 1,750 passed tests, 3 skipped; Vite production build; factory-readiness passed.
- Fifth review-fix RED: added prepared-only readback coverage for missing/malformed manifests, missing final-output binding, bad prepared causation, and nondeterministic handoff idempotency keys. `npm test -- packages/ontology/test/agent-contracts.test.ts packages/agent/test/specialist-handoff-projection.test.ts packages/agent/test/projection.test.ts packages/agent/test/cockpit.test.ts` failed as expected with 3 assertions: unsafe prepared-only state projected `handoff-pending`, nondeterministic projector idempotency projected `handoff-recorded`, and ontology event validation accepted the bad idempotency key.
- Fifth review-fix GREEN: the same targeted command passed with 4 test files and 91 tests after prepared-only state loaded and verified the manifest before `handoff-pending`, and after ontology/projector validation enforced the deterministic handoff idempotency key.
- Fifth review-fix verification: initial `npm run verify` caught an exact-optional TypeScript issue in the ontology idempotency helper; after tightening the helper type, `npm run verify` passed: typecheck; 172 passed test files, 3 skipped; 1,752 passed tests, 3 skipped; Vite production build; factory-readiness passed.
