...
Command evidence:
- Red: `npm test -- packages/agent/test/provider.test.ts` failed before implementation with `Cannot find module '../src/provider.js'` from `packages/agent/test/provider.test.ts`.
- Green: `npm test -- packages/agent/test/provider.test.ts` passed with 1 test file and 3 tests passing.
- Targeted: `npm test -- packages/agent/test/provider.test.ts packages/agent/test/projection.test.ts` passed with 2 test files and 9 tests passing.
- Verify: `npm run verify` passed with typecheck passed, 107 test files and 993 tests passing, UI build succeeded, and factory-readiness passed.

Self-review notes:
- Provider descriptors contain provider metadata only and no `residentAgentId`.
- Credential references validate IDs, provider IDs, kinds, and optional safe labels without accepting raw credential values.
- `FakeModelProvider` is deterministic and local-only; it performs no network, browser, or environment credential access.
- No provider byte transfer, PRR send, legal escalation, export, repair, or accepted graph-review execution path was added.
