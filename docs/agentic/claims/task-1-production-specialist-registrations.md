...
- Specialist authority synonym review status: `ready-for-review`.
- Specialist authority synonym review RED: `npm test -- packages/agent/test/production-specialist-prompts.test.ts` failed with 1 expected regression and 27 passing tests because the new exact authority-synonym cases, beginning with `The PRR was dispatched.`, were accepted. The focused regression coverage also includes `The provider transfer was completed.` and `The repair ran successfully.`.
- Specialist authority synonym review GREEN: the same focused command passed with 1 file and 28 tests after shared authority matching recognized dispatched PRR sends, provider-transfer completion with or without `byte`, and repairs that ran successfully. Existing safe ordinary-evidence coverage remains accepted.
- Specialist authority synonym review verify: `npm run verify` passed: typecheck, 178 passed / 3 skipped test files, 1,974 passed / 3 skipped tests, Vite build, and factory readiness. Node emitted only its existing experimental SQLite warnings; Vite emitted its existing `node:crypto` externalization and chunk-size warnings.
