...
- Active export/publication authority review RED: `npm test -- packages/agent/test/production-specialist-prompts.test.ts` failed with 2 expected regressions and 28 passing tests because `I published the report.` and `We exported the evidence.` were accepted.
- Active export/publication authority review GREEN: the same focused command passed with 1 file and 30 tests after export/publication matching recognized first-person completed active voice acting on report/evidence subjects; `The report cites a published 2023 public record.` remains accepted.
- Active export/publication authority review verify: `npm run verify` passed: typecheck, tests, Vite build, and factory readiness. Node emitted only the existing experimental SQLite warnings.
- Provider transfer performed review RED: `npm test -- packages/agent/test/production-specialist-prompts.test.ts` failed with 1 expected regression and 29 passing tests because `The provider byte transfer was performed.` was accepted as a completed provider byte-transfer claim.
- Provider transfer performed review GREEN: the same focused command passed with 1 file and 30 tests after provider-transfer authority matching rejected `performed` alongside existing completion actions without changing policy/evidence treatment.
- Provider transfer performed review verify: `npm run verify` passed: typecheck, 178 passed / 3 skipped test files, 1,976 passed / 3 skipped tests, Vite build, and factory readiness. Node emitted its existing experimental SQLite warnings; Vite emitted its existing `node:crypto` externalization and chunk-size warnings.
- Runtime run-type guard review RED: `npm test -- packages/agent/test/production-specialist-prompts.test.ts` failed with 2 expected regressions and 30 passing tests because cast `ontology-bootstrap` and arbitrary run types returned `undefined` rather than rejecting at the untrusted provider-output boundary.
- Runtime run-type guard review GREEN: the same focused command passed with 1 file and 32 tests after the dispatch switch fail-closed for every unrecognized runtime run type before parsing its payload, using a non-sensitive unsupported-run-type error.
- Runtime run-type guard review verify: `npm run verify` passed; Node emitted only its existing experimental SQLite warnings.
