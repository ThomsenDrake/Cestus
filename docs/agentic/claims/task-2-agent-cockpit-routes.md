...
- Generic `POST /api/agent/runs` is not exposed by the corrected cockpit slice and must not validate, append, schedule, or execute generic specialist work.
...
Command evidence:
- Red: `npm test -- packages/local-runtime/test/agent-cockpit-routes.test.ts` failed before implementation with route coverage missing for cockpit state and generic run-route absence.
...
- Scheduler wake, approval routes, memory routes/provenance, and the ontology-bootstrap specialist-specific executable route are preserved; no specialist execution, provider invocation, PRR send, provider byte transfer, export, legal escalation, repair, accepted graph review, legacy import or staging, or workspace mutation paths were added to generic cockpit routes.
