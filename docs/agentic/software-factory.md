...
`docs/agentic/contracts/software-factory-active-mission.v1.json` is the sole machine-readable selector for current work. Factory readiness authenticates that selector before it authenticates the selected checker; the checker independently pins the active immutable envelope. The selector preserves the unchanged integrated `software-factory-calibration` predecessor and selects `docs/agentic/contracts/resident-agent-full-vision-mission-state.v1.json` as the active Level 3 mission. The active checker derives the complete remaining order, dependency and milestone eligibility, strictly ordered shared ownership, E1328 verifier ownership, fingerprints, and compact status. Eligibility means dependency-ready for a separate append-only lifecycle authorization; it never creates a `claimed` event.

The historical `docs/agentic/contracts/software-factory-mission-state.v1.json`, its checker, and its checker tests remain byte-frozen. They continue to define Level 1–3 mechanics and to reference frozen V4 release authority without copying its graph. The successor references the committed full-vision design and plan, CF-1 freeze and terminal amendments, acceptance design, plan and matrix, the E1328 snapshot, and E1329 authorization rather than changing that predecessor envelope. A candidate control plane reports prospective readiness only and cannot advance product state. Projected reviewed integrations and milestone gates are valid only when pairwise-distinct exact machine markers exist in unique append-only registry event sections at authenticated registry-only commits; milestone closure additionally resolves separate black-box and full-gate evidence commits before the gate event. Mutable event-ID strings or reachable SHAs alone cannot advance eligibility.
...
- Gatekeeper: runs the source-defined targeted and risk-proportionate gates, including factory readiness when required.

## Calibrated Durable Task State

The mission source is the compact durable record for active ownership and status. Level 1 work does not create a claim, permanent RED commit, full `npm run verify`, or fresh review by default. Use those controls only when the selected level or task contract requires them.

When a Level 2 or Level 3 task needs a claim, use a concise claim file with the source-defined lifecycle state and owned paths. Do not add routine command, polling, waiting, unchanged-state, or reviewer-heartbeat records.

## Work Order Lifecycle

1. Select the task's source-defined risk level and inspect the owned boundary.
2. Use a failing test or exact reproduction before behavior changes; use focused validation for Level 1 documentation and behavior-neutral work.
3. Implement the smallest scoped change and run the source-defined validation gates.
4. Make an atomic commit and obtain only the source-defined review and integration gate.
