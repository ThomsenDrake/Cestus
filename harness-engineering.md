# Implementing Symphony-Inspired Orchestration in Cestus
...
- What Cestus already exposes as “execution primitives” (solve entrypoints, session/workspace model, event streaming and persistence, tool schema system). fileciteturn98file0L1-L1 fileciteturn105file0L1-L1 fileciteturn104file0L1-L1 fileciteturn90file0L1-L1
- How Cestus desktop bridges engine telemetry to the UI (Tauri invoke routes, event channels, logging emitter, UX state store). fileciteturn94file0L1-L1 fileciteturn103file0L1-L1 fileciteturn104file0L1-L1 fileciteturn93file0L1-L1 fileciteturn101file0L1-L1
- What CI/test harness already exists in Cestus (to propose iteration-speed improvements without fighting the repo). fileciteturn95file0L1-L1 fileciteturn97file0L1-L1 fileciteturn92file0L1-L1
...
The highest leverage path is to add an orchestration subsystem to Cestus desktop (Rust `op-core` + Tauri + frontend), while optionally adding a parallel CLI entry point later. That yields:
...
- User-facing power: “issue-driven automation”, per-task workspaces, tracker adapters, notifications, remote worker support, and an orchestrator dashboard inside the existing Cestus UI event model. fileciteturn104file0L1-L1 fileciteturn93file0L1-L1
...
### What Cestus provides, concretely

Cestus already has the “agent runtime UX contract” needed for orchestration:
...
- **Multiple entrypoints**: Python CLI (`cestus-agent = agent.__main__:main`) and the desktop app (Tauri invoke handler). fileciteturn97file0L1-L1 fileciteturn98file0L1-L1 fileciteturn101file0L1-L1

The missing pieces are almost entirely “orchestration and tracker integration”, not “agent runtime”.

## Mapping Symphony concepts into Cestus with concrete change proposals
...
| Dynamic tools | Add tracker tools (`github_graphql`, `linear_graphql`) into Cestus’s tool registry; gate unsafe tools | Agent can read/write tracker state autonomously | Extend `agent/tool_defs.py` dynamic tool merge already exists fileciteturn90file0L1-L1; mirror Symphony dynamic tool contract fileciteturn88file0L1-L1 | Small–Medium |

### Concrete implementation details by component

#### Orchestrator

**Goal:** Recreate Symphony’s “poll -> dispatch -> track running -> retry/backoff -> stall restart” loop inside Cestus. Symphony’s orchestrator state includes `running`, `retry_attempts`, and concurrency config, and refreshes config as it ticks. fileciteturn82file0L1-L1

**Where to add in Cestus (desktop path):**
...
**Cestus insertion point:** implement `AgentRunner::run_issue(issue, cfg)` as a wrapper around the existing “solve objective” pipeline, and expose it to the orchestrator.

**Concrete Cestus locations to integrate:**
...
- Desktop UI can display each run as a “turn” within an issue, similar to how Cestus already displays step/turn progress for a single run via events. fileciteturn105file0L1-L1 fileciteturn103file0L1-L1
...
- Introduce an optional `WORKFLOW.md` in the Cestus workspace root (or `.openplanter/WORKFLOW.md` to avoid collisions). Reuse the same “YAML frontmatter then markdown template” pattern.
...
**Cestus should implement:**

- `trait TrackerAdapter` (Rust op-core)
- `GitHubAdapter` first (because you explicitly want it to increase Cestus power, and Cestus already has GitHub-based distribution and CI patterns). fileciteturn95file0L1-L1
...
**Dynamic tool integration inside Cestus:**

Cestus’s tool definition system supports merging arbitrary `dynamic_defs` into the tool list while keeping strict schemas. fileciteturn90file0L1-L1
...
This is the one place where Cestus is structurally different from Symphony. Symphony uses a Codex app-server JSON-RPC stream over stdio, and treats tool calls + approvals as protocol events. fileciteturn87file0L1-L1

Cestus already has a stable event and persistence layer; the missing piece is a “headless server mode” so the desktop UI is not the only controller.

**Proposed Cestus addition:**

- Add an optional local daemon (“Cestus app-server”) that exposes:
...
Symphony creates per-issue workspaces under a configured root and validates they remain inside it (prevent escapes, symlink tricks). fileciteturn85file0L1-L1 Cestus already uses a related guardrail: reject repo root and redirect to `workspace/`. fileciteturn77file0L1-L1 fileciteturn102file0L1-L1
...
This makes Cestus immediately more powerful for user-facing “task automation”: every issue gets a clean sandbox.
...
- Add a new UI pane:
  ...
  - include “open workspace” and “open replay” affordances (Cestus already persists replay/events). fileciteturn104file0L1-L1
...
- Minimum viable SSH support:
  - Run hooks remotely
  - Run solve remotely by calling the new “Cestus daemon” on the remote machine (simplifies log streaming)
- Emit worker host and workspace path in orchestrator events (already done in Symphony). fileciteturn82file0L1-L1

#### Dynamic tools

Cestus’s tool definition layer already supports dynamic tool definitions injection without changing base tools. fileciteturn90file0L1-L1 Symphony uses this to expose tracker access (`linear_graphql`). fileciteturn88file0L1-L1

**Concrete Cestus improvement:**
...
Cestus desktop already has a clean, observable execution pipeline:
...
Cestus CLI already centralizes its bootstrapping and run modes in `agent/__main__.py`, which makes it the natural place to add `orchestrate` subcommands. fileciteturn98file0L1-L1

## Gaps, security/sandboxing concerns, and validation plan

### Gaps and divergences vs Symphony

- **No tracker integration boundary** in the inspected Cestus surface area, while Symphony makes tracker adapters a first-class abstraction. fileciteturn84file0L1-L1
- **No long-lived orchestrator loop** in Cestus, while Symphony’s core runtime is the orchestrator. fileciteturn82file0L1-L1
...
- **Approval/sandbox policy mismatch**: Symphony’s workflow config encodes explicit “approval_policy” and sandbox settings for unattended runs. fileciteturn81file0L1-L1 Cestus can execute powerful tools and should introduce a stricter unattended profile before enabling autonomous tracker actions. fileciteturn90file0L1-L1

### Security and sandboxing priorities

If you turn Cestus into an unattended task runner, the primary risk is not the model, it is the effects of tools and credentials.
...
Cestus already runs Python, Rust, and frontend tests in CI. fileciteturn95file0L1-L1 The plan below adds orchestration coverage with minimal friction.
...
- Proposed:
  - `cestus-agent orchestrate --workflow WORKFLOW.md` (new CLI mode) fileciteturn98file0L1-L1
