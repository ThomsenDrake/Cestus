...
test("binds four historical source records and exact record-11 and record-14 current-head migrations", () => {
  ...
  assert.equal(parsedPrefix.length, 22);
  assert.deepEqual(parsedPrefix.map((record) => record.cardId), expectedIds.slice(0, 22));
  assert.equal(
  ...
  assert.throws(
    ...
    /repository release closure incomplete: expected 29 records, found 22/
  );
  assert.deepEqual(successfulMessages, ["TASK136_REPOSITORY_PREFIX_OK records=22 commands=22"]);
  assert.equal(successfulAdapter.commandCalls.length, 22);
...
test("requires the finite Task137B-W to Task139-PM transfer only at record 18", () => {
  ...
  assert.equal(releasedPrefix.length, 22);
  assert.deepEqual(releasedPrefix.map((record) => record.cardId), expectedIds.slice(0, 22));
  for (const [cardId, expectedHash] of rawPrefixPins) {
    assert.equal(createHash("sha256").update(rawRecordJson(cardId)).digest("hex"), expectedHash, cardId);
  }
