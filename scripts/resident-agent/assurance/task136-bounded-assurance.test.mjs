...
test("binds four historical source records and exact record-11 and record-14 current-head migrations", () => {
  ...
  assert.equal(parsedPrefix.length, 19);
  assert.deepEqual(parsedPrefix.map((record) => record.cardId), expectedIds.slice(0, 19));
  assert.equal(
  ...
  assert.deepEqual(successfulMessages, ["TASK136_REPOSITORY_PREFIX_OK records=19 commands=19"]);
  assert.equal(successfulAdapter.commandCalls.length, 19);
...
test("requires the finite Task137B-W to Task139-PM transfer only at record 18", () => {
  ...
  assert.equal(releasedPrefix.length, 19);
  assert.deepEqual(releasedPrefix.map((record) => record.cardId), expectedIds.slice(0, 19));
  for (const [cardId, expectedHash] of rawPrefixPins) {
    assert.equal(createHash("sha256").update(rawRecordJson(cardId)).digest("hex"), expectedHash, cardId);
  }
