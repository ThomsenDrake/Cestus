...
export function buildOntologyWorkspaceReadDto(
  ...
  const relationshipEvents = new Map<
    string,
    KnowledgeEventOf<"relationship.accepted">
  >();
  ...
  for (const event of events) {
    if (event.type === "assertion.proposed") {
      if (assertionStates.has(event.payload.assertionId)) {
        diagnostics.push(
          missingProvenanceDiagnostic(
            "A repeated assertion proposal requires a new exact acceptance before it can enter accepted truth.",
          ),
        );
      }
      ...
      if (
        state !== undefined &&
        event.context.causationId === state.proposal.id
      ) {
        state.acceptance = event;
      } else {
        diagnostics.push(
          missingProvenanceDiagnostic(
            "An assertion acceptance was omitted because its proposal provenance is incomplete.",
          ),
        );
      }
  ...
  const assertions = [...assertionStates.values()]
    .sort((left, right) =>
      left.proposal.payload.assertionId.localeCompare(
        right.proposal.payload.assertionId,
      ),
    )
    .map((state): OntologyAssertionReadDto => {
      const proposal = state.proposal;
      const causedBy =
        proposal.context.causationId === undefined
          ? undefined
          : eventsById.get(proposal.context.causationId);
      const evidence =
        causedBy?.type === "evidence.ingested" &&
      ...
      if (!evidenceIsBound) {
        diagnostics.push(
          missingProvenanceDiagnostic(
            "An assertion was omitted from accepted truth because its evidence event provenance is incomplete.",
          ),
        );
      }

      const reviewState =
        accepted !== undefined && evidenceIsBound ? "accepted" : "proposed";
      const relatedEvents = [
        evidence,
        proposal,
        reviewState === "accepted" ? accepted : undefined,
      ].filter(
        (
          event,
        ): event is
  ...
  const assertionById = new Map(
    assertions.map((assertion) => [assertion.assertionId, assertion]),
  );
  ...
  for (const event of entityEvents.values()) {
    ...
    if (
      projected === undefined ||
      !event.payload.assertionIds.every((assertionId) =>
        acceptedAssertionIds.has(assertionId),
      )
    ) {
      diagnostics.push(
        missingProvenanceDiagnostic(
          "A resolved entity was omitted because accepted assertion provenance is incomplete.",
        ),
      );
      continue;
    }
    ...
    const supportingAssertions = event.payload.assertionIds
      ...
      .filter(
        (assertion): assertion is OntologyAssertionReadDto =>
          assertion !== undefined,
      );
    entities.push({
      ...
      evidenceIds: sortedUnique(
        supportingAssertions.map((assertion) => assertion.evidenceId),
      ),
  ...
  for (const event of relationshipEvents.values()) {
    ...
    if (
      ...
      diagnostics.push(
        missingProvenanceDiagnostic(
          "An accepted relationship was omitted because endpoint or assertion provenance is incomplete.",
        ),
      );
      continue;
    }
    ...
    const contradictionAssertions = assertions.filter(
      (assertion) =>
        assertion.subjectRef === event.payload.relationshipId &&
        isContradictionPredicate(assertion.predicate),
    );
    const supportingAssertions = referencedAssertions.filter(
      (assertion) => !isContradictionPredicate(assertion.predicate),
    );
    const allRelevantAssertions = uniqueAssertions([
      ...referencedAssertions,
      ...contradictionAssertions,
    ]);
    ...
    relationships.push({
      ...
      reviewState: "accepted",
      supportingAssertionIds: sortedUnique(
        supportingAssertions.map((assertion) => assertion.assertionId),
      ),
      ...
      evidenceIds: sortedUnique(
        allRelevantAssertions.map((assertion) => assertion.evidenceId),
      ),
      eventIds: sortedUnique([
        event.id,
  ...
  // v2 shares the same current graph; legacy DTOs gain actual values and honest citation scope.
  for (const proposal of graph.knowledge.proposals.filter(
    (p) => p.reviewState === "proposed" || p.reviewState === "accepted",
  )) {
    const current = graph.assertions.get(proposal.assertionId);
    assertions.push({
      assertionId: proposal.assertionId,
      reviewState: current ? "accepted" : "proposed",
      ...(proposal.subjectEntityId
        ? { subjectRef: proposal.subjectEntityId }
        : {}),
      predicate: proposal.predicate,
      confidence: proposal.modelScore ?? 0,
      object:
        proposal.value.type === "entity"
          ? (proposal.objectEntityId ?? null)
          : proposal.value.value,
      citationScope: "passage",
      evidenceId: proposal.evidence[0]!.evidenceId,
      eventIds: sortedUnique([
        proposal.proposalEventId,
        ...proposal.history.map((h) => h.eventId),
        ...proposal.evidence.flatMap((e) => e.provenanceEventIds),
      ]),
      packVersions: [{ name: "knowledge", version: proposal.schemaId }],
    });
  }
  for (const entity of graph.knowledge.entities) {
    if (entityEvents.has(entity.entityId)) continue;
    if (entities.some((existing) => existing.entityId === entity.entityId))
      continue;
    const support = assertions.filter((assertion) =>
      entity.assertionIds.includes(assertion.assertionId),
    );
    entities.push({
      ...
      reviewState: "accepted",
      supportingAssertionIds: entity.assertionIds,
      evidenceIds: entity.evidenceIds,
      eventIds: sortedUnique(
        support.flatMap((assertion) => assertion.eventIds),
      ),
      packVersions: mergePackVersions(
        support.map((assertion) => assertion.packVersions),
      ),
    });
  }
  for (const relationship of graph.knowledge.relationships) {
    if (relationshipEvents.has(relationship.relationshipId)) continue;
    if (
      relationships.some(
        (existing) => existing.relationshipId === relationship.relationshipId,
      )
    )
      continue;
    const support = assertions.filter((assertion) =>
      relationship.assertionIds.includes(assertion.assertionId),
    );
    relationships.push({
      ...
      reviewState: "accepted",
      supportingAssertionIds: relationship.assertionIds,
      contradictingAssertionIds: [],
      evidenceIds: sortedUnique(
        support.map((assertion) => assertion.evidenceId),
      ),
      eventIds: sortedUnique(
        support.flatMap((assertion) => assertion.eventIds),
      ),
  ...
  return degradedReadDto(0, [
    {
      ...
      repairActions: [
        "inspect local ledger diagnostics",
        "retry the ontology replay",
      ],
    },
  ]);
}
...
function hasProjectionLag(events: readonly KnowledgeEvent[]): boolean {
  const latestCheckpoint = [...events]
    ...
    .find(
      (event): event is KnowledgeEventOf<"projection.checkpointed"> =>
        event.type === "projection.checkpointed" &&
        event.payload.projectionName === "ontology-graph",
    );
  ...
  const latestOntologySourceOrdinal = events.reduce(
    (ordinal, event, index) =>
      ontologySourceEventTypes.has(event.type) ? index + 1 : ordinal,
    0,
  );
  return (
    latestCheckpoint.payload.status !== "ready" ||
    latestCheckpoint.payload.highWaterMark < latestOntologySourceOrdinal
  );
}

function packVersionsFor(
  events: readonly KnowledgeEvent[],
): OntologyPackVersionDto[] {
  const versions: OntologyPackVersionDto[][] = events.map((event) => {
    const entries = Object.entries(event.context.packVersions).map(
...
function mergePackVersions(
  versionGroups: readonly (readonly OntologyPackVersionDto[])[],
): OntologyPackVersionDto[] {
...
function uniqueAssertions(
  assertions: readonly OntologyAssertionReadDto[],
): OntologyAssertionReadDto[] {
  return [
    ...new Map(
      assertions.map((assertion) => [assertion.assertionId, assertion]),
    ).values(),
...
function projectionLagDiagnostic(): OntologyReadDiagnosticDto {
  return {
    ...
    repairActions: [
      "rebuild the ontology projection from the ledger",
      "retry after the projection is ready",
    ],
  };
}
...
function unknownEventDiagnostic(): OntologyReadDiagnosticDto {
  return {
    ...
    severity: "error",
    message:
      "The ontology replay encountered an unknown or invalid ledger event.",
    repairActions: [
      "inspect the event contract version",
      "repair the ledger reader before replaying",
    ],
  };
}
...
function missingProvenanceDiagnostic(
  ...
  return {
    ...
    message,
    repairActions: [
      "inspect the referenced ledger events",
      "rebuild after provenance is restored",
    ],
  };
}
