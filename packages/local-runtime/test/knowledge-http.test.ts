...
it("exposes canonical passage citations through the production evidence route", async () => {
  ...
  writeFileSync(join(sourceRoot, "record.txt"), "Alex Vale received 1200 in 2025-03. Registered September 5, 2026.\n");
  ...
  expect(content.citations[0]).toMatchObject({ workspaceId: "ws_knowledge_test", quote: "Alex Vale received 1200 in 2025-03. Registered September 5, 2026.", passageIndex: 0 });
  ...
  await command({ action: "extendSchema", baseSchemaId: "investigation.v1", addition: { kind: "predicate", definition: { name: "registered_on", kind: "fact", valueType: "date", fromTypes: ["person"], toTypes: [] } }, rationale: "Reviewed date meaning", actorId: "spoofed" }, 409);
  await command({ action: "extendSchema", baseSchemaId: "investigation.v1", addition: { kind: "predicate", definition: { name: "registered_on", kind: "fact", valueType: "date", fromTypes: ["person"], toTypes: [] } }, rationale: "Reviewed date meaning" });
  const written = { ...fact, assertionId: "as_written_date", predicate: "registered_on", schemaId: "investigation.v2", value: { type: "date", value: "2026-09-05", normalization: { method: "written-date.v1", sourceExpression: "September 5, 2026", citationIndex: 0 } } };
  await command({ action: "propose", proposals: [{ ...written, value: { ...written.value, value: "2026-09-06" } }] }, 409);
  await command({ action: "propose", proposals: [written] });
  const writtenReview = (await request("/api/ontology/knowledge")).proposals.find((p: { assertionId: string }) => p.assertionId === written.assertionId);
  await command({ action: "review", reviews: [{ assertionId: written.assertionId, proposalEventId: writtenReview.proposalEventId, action: "accept", rationale: "Inspected written source and normalized day" }] });
  const normalized = await request("/api/ontology/knowledge");
